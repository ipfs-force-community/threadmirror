...
func (x *XScraper) GetGraphQL(ctx context.Context, endpoint string, params interface{ Query() url.Values }, target any) error {
	return x.DoGraphQL(ctx, http.MethodGet, endpoint, params, target)
}

func (x *XScraper) PostGraphQL(ctx context.Context, endpoint string, params interface{ Query() url.Values }, target any) error {
	return x.DoGraphQL(ctx, http.MethodPost, endpoint, params, target)
}

var errNotLoggedIn = errors.New("not logged in")

func (x *XScraper) DoGraphQL(ctx context.Context, method, endpoint string, params interface{ Query() url.Values }, target any) error {
	// 初始化登录（只执行一次）
	...
	for {
		...
		// 执行实际请求
		err := x.doGraphQL(ctx, method, endpoint, params, target)
