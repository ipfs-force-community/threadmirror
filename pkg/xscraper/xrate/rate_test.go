...
func TestResetAt(t *testing.T) {
	lim := NewLimiter(Every(1*time.Second), 1)
	if lim.Tokens() != 1 {
		t.Errorf("Limit(1, 1) want 0 when first used")
	}
	resetAt := time.Now().Add(time.Hour)
	lim.ResetAt(resetAt, 0)
	if lim.Tokens() != 0 {
		t.Errorf("After reset a future time, Tokens should be 0")
	}
	if lim.TokensAt(resetAt) != 0 {
		t.Errorf("After reset a future time, TokensAt should be 0")
	}
	if lim.TokensAt(resetAt.Add(time.Second)) <= 0 {
		t.Errorf("After reset a future time, TokensAt should be gt 0")
	}
}
